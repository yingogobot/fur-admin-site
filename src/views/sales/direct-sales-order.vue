<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-select v-model="listQuery.sales_channel_id" placeholder="销售渠道" clearable style="width: 150px; margin-left: 15px;" class="filter-item">
      ...
      <el-button v-if="role === 7 || role === 1" class="filter-item" style="width: 200px; margin-left: 10px;" type="success" icon="el-icon-edit" @click="handleCreate">
        添加新销售
      </el-button>
    </div>
    ...
    >
      ...
      <el-table-column label="销售平台" prop="sales_channel" width="100px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.sales_channel_title }}</span>
        </template>
      </el-table-column>
      <el-table-column label="销售时间" prop="sales_date" width="100px" align="center">
        <template slot-scope="{row}">
          <span>{{moment(row.date).format('YYYY-MM-DD')}}</span>
        </template>
      </el-table-column>
      <el-table-column label="客户信息" prop="sales_member" width="180px" align="center">
        <template slot-scope="{row}">
      ...
      <el-table-column v-if="role === 7 || role === 1" label="编辑订单" width="150px" align="center">
...
export default {
  ...
  methods: {
    ...
    getAllSales() {
      ...
      SalesAPI.getAllDirectSales(this.listQuery)
        .then(response => {
    ...
    getSalesCount() {
      SalesAPI.getAllDirectSalesCount(this.listQuery)
        .then(response => {
    ...
    objectSpanMethod({ row, column, rowIndex, columnIndex }) {
      if (columnIndex < 4 || (columnIndex >= 13 && columnIndex <= 22)) {
        let data = {
          rowspan: 0,
          colspan: 0
        };
